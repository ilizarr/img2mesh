...
def render(height, width, vertices, mesh_shift, vertex_colors, faces, cam_transform, cam_proj):
    ...
    image = torch.clamp(image_color * mask, 0., 1.)
...
def save_plots(dirname, epoch_num, data_set, elem_ids, model, vertices_init, faces):
    ...
    with torch.no_grad():
        ...
        gt_images = torch.stack([data['rgb'].cpu().detach() for data in samples], dim=0).cuda()
        orig_image = torch.stack([data['rgb_orig'].cpu().detach() for data in samples], dim=0).cuda()
        height, width = 1024,1024#gt_image.shape[-2], gt_image.shape[-1]
...
image_weight = 0.1
mask_weight = 1.
lr = 1e-4 if not skip_model else 5e-2
scheduler_step_size = 15
scheduler_gamma = 0.5

# Load data
batch_size = 3 if not skip_model else 1
views_per_batch = 2
...
vertices_laplacian_matrix = kal.ops.mesh.uniform_laplacian(nb_vertices, faces)
...
if not skip_model:
    ...
    vertices_alt = torch.tensor(mesh.vertices).cuda().unsqueeze(0) * 75
...
for epoch in range(num_epoch):
    for _, data in enumerate(dataloader_train):
        ...
        gt_image = data['rgb'].cuda() # (N*V x C x H x W)  
        orig_image = data['rgb_orig'].cuda() 
        height, width = 1024,1024#gt_image.shape[-2], gt_image.shape[-1]

        if not skip_model:
            # Split by the number of views processed in parallel
            x = gt_image.view(batch_size, views_per_batch, gt_image.shape[-3], gt_image.shape[-2], gt_image.shape[-1])
